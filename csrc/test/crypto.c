/*
 * CRC16-CCITT-FALSE and SHA-256, for the test harness only. In the openambit tree
 * these functions come from crc16.c and sha256.c; they are rewritten here so the
 * serializer can be built without copying GPL code into this repository.
 */
...
{
    return crc16_ccitt_false_init(buf, buflen, 0xffff);
}
...
void sha256(const uint8_t *data, size_t len, uint8_t *hash)
{
    ...
    for (i = 0; i < tail_len / 64; i++) {
        sha256_block(h, tail + i * 64);
    }
    for (i = 0; i < 8; i++) {
